...
PROMPTS = {
    LLMType.COHERE: {
        PromptType.SEMANTIC_PARSING: """Parse the following search query into a structured format that includes entities (including person names) and a MongoDB-like boolean query:
...
Output:""",
        PromptType.SIMILAR_QUERY: """Generate {k} very similar search queries with alternate words to the following query, ignore all the person names and first-person context.
...
def get_prompt(llm: LLMType, prompt_type: PromptType, **kwargs) -> str:
    return PROMPTS[llm][prompt_type].format(**kwargs)
