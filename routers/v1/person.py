...
from models.person import Person, PersonResponse
...
async def create_person(person: PersonResponse):
    with MongoConnection(collection=MongoCollections.PERSON) as conn:
        conn.insert(person.model_dump())

    return person


@router.get("/search", response_model=list[PersonResponse], status_code=status.HTTP_200_OK)
async def search_persons(name: str, user=Depends(get_current_user)):
    print(name)
    with MongoConnection(collection=MongoCollections.PERSON) as conn:
        persons = conn.find_many({"name": {"$regex": name, "$options": "i"}, "owner": user.id})
...
async def get_person(person_id: str, user=Depends(get_current_user)):
    with MongoConnection(collection=MongoCollections.PERSON) as conn:
        person = conn.find({"id": person_id, "owner": user.id}, fields={"centroid": 0})
...
@router.put(
    "/{person_id}/name/{name}", response_model=PersonResponse, status_code=status.HTTP_200_OK
)
...
async def list_persons(limit: int = 100, page: int = 0, user=Depends(get_current_user)):
    with MongoConnection(collection=MongoCollections.PERSON) as conn:
        persons = conn.find_many({"owner": user.id}, limit=limit, page=page, fields={"centroid": 0})
